...
if __name__ == "__main__": exists
    ...
    def _start_background_thread(self) -> None:
        ...
        logger.info("Started background task thread")
    ...
    def _background_task_loop(self) -> None:
        ...
        while self.bg_running:
            try:
                ...
                time.sleep(1)  # Avoid hammering if there's a persistent error
...
"""Language: {language}    return f"""{prompt}    """Create prompt for code generation"""def _create_generation_prompt(self, prompt: str, language: str) -> str:    return result        }        "timestamp": time.time()        "language": language,        "explanation": explanation,        "generated_code": code,        "prompt": prompt,    result = {    # Build result        code, explanation = self._parse_generation(generated_text, language)    # Parse generated code            )            top_p=0.95            temperature=temperature,            max_new_tokens=max_tokens,            prompt=code_prompt,        generated_text = self.generator.generate(    with self.lock:    # Generate code        code_prompt = self._create_generation_prompt(prompt, language)    # Create prompt            raise RuntimeError("Model is not loaded")    if not self.status["model_loaded"]:    # Check if model is loaded    """        Dictionary with generated code and explanation    Returns:                temperature: Sampling temperature        max_tokens: Maximum number of tokens to generate        language: Target programming language        prompt: Natural language prompt describing the code to generate    Args:        Generate code based on a natural language prompt    """                temperature: float = 0.7) -> Dict[str, Any]:                max_tokens: int = 1024, def generate_code(self, prompt: str, language: str,     return enhanced_code, explanation            # This would need language-specific heuristics        explanation = enhancement_text    if not enhanced_code:    # If no code block was found, try to extract code by looking for language keywords                    break                explanation = "\n\n".join(explanation_parts)                                        explanation_parts.append(part.strip())                    if part.strip():                        part = part.split("```", 1)[-1]                    if "```" in part:                    # Skip any additional code blocks                for part in parts[2:]:                                    explanation_parts.append(parts[0].strip())                if parts[0].strip():                explanation_parts = []                # Extract explanation (combine text before and after code block)                                enhanced_code = parts[1].strip()                # Extract code from middle section            if len(parts) >= 3:            parts = enhancement_text.split(marker)        if marker in enhancement_text:    for marker in code_block_markers:    # Extract code from code blocks        explanation = enhancement_text    enhanced_code = ""        ]        "```java", "```typescript", "```c++", "```cpp"        f"```{language}", "```", "```python", "```javascript",     code_block_markers = [    # Look for code blocks    """Parse enhanced code from enhancement text"""def _parse_enhancement(self, enhancement_text: str, language: str) -> Tuple[str, str]:    from transformers import AutoTokenizer, AutoModelForCausalLM
    ...
    # Load a small model for demonstration
        ...
        print(f"  - {citation}")    for citation in citations:    print(f"Found {len(citations)} citations:")
    citations = legal_assistant.extract_citations(sample_text)
    print("\nExtracting citations from sample text...")
    
    """
    510 U.S. 569 (1994), the Court emphasized the importance of transformative use.    model_name = "deepseek-ai/deepseek-coder-1.3b-instruct"
    tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True)
    model = AutoModelForCausalLM.from_pretrained(


    established the concept of "time shifting" as fair use. Later, in Campbell v. Acuff-Rose Music,     The court in Sony Corp. of America v. Universal City Studios, Inc., 464 U.S. 417 (1984), 
    sample_text = """
    # Extract citations from a sample text
    ...
    # Move model to appropriate device            model_name, 
        trust_remote_code=True,
        torch_dtype=torch.float16 if torch.cuda.is_available() else torch.float32
    )