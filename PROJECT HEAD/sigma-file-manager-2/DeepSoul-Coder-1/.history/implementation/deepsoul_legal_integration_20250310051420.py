...
"""
    ...
    def _parse_enhancement(self, enhancement_text: str, language: str) -> Tuple[str, str]:
        """Parse enhanced code and explanation"""
        # This implements the same logic as _parse_generation
        return self._parse_generation(enhancement_text, language)
    ...
    def _create_knowledge_extraction_prompt(self, content: str, language: str, file_path: str) -> str:
        """Create prompt for knowledge extraction"""
        return f"""Extract knowledge from the following {language} file:
...
"""
    ...
    def _parse_knowledge_items(self, analysis_text: str, source: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            if line.strip():
                items.append({"source": source, "content": line.strip()})
