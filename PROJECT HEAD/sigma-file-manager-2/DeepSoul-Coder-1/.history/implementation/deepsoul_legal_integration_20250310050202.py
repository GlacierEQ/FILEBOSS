...
def demo():
    ...
    _markers = [
    print("Initializing legal assistant...")hon", "```javascript", 
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        torch_dtype=torch.float16 if torch.cuda.is_available() else torch.float32 blocks
    )
     generation_text:
    # Move model to appropriate deviceparts = generation_text.split(marker)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = model.to(device)
    
    # Create legal assistant
    legal_assistant = LegalAssistant(bine text before and after code block)
        model=model, = []
        tokenizer=tokenizer,strip():
        output_dir="legal_assistant_demo"ion_parts.append(parts[0].strip())
    )
    2:]:
    # Research a legal topicnal code blocks
    print("\nResearching legal topic: 'fair use copyright'...")n part:
    results = legal_assistant.research_legal_topic(
        topic="fair use copyright",
        depth=1 tion_parts.append(part.strip())
    )    
    ation_parts)
    # Print summary if available
    if results.get("summary"):
        print(f"Summary: {results['summary']}")on
    else:
        print("No summary available.")r, source_type: str = "auto") -> List[Dict[str, Any]]:
    ...
    for citation in citations:
        print(f"  - {citation}")
        ._determine_source_type(source)")
    print("\nDemo completed!")
ype
if __name__ == "__main__":
    demo()    return self._acquire_from_file(source)e__ == "__main__":
