...
class LegalAssistant:
    ...
    def analyze_legal_document(self, analysis_type: str = "summary") -> Dict[str, Any]:
        """
...
"""
    ...
    def enhance_code(self, code: str, language: str, 
        ...
        # Create prompt
        prompt = self._create_enhancement_prompt(code, enhancement_type)
    ...
    def _create_enhancement_prompt(self, code: str, enhancement_type: str) -> str:
...
"""
    ...
    def _extract_code_and_explanation(self, parts: List[str]) -> Tuple[str, str]:
        """Extract code and explanation from parts of a split text"""
        code = parts[1].strip()
        explanation_parts = []
        ...
        # Process parts after the code block
        for part in parts[2:]:
            if "```" in part:
                part = part.split("```", 1)[-1]
            if part.strip():
    ...
    def _parse_generation(self, generation_text: str) -> Tuple[str, str]:
        ...
        for marker in code_block_markers:
            if marker in generation_text:
                ...
                if len(parts) >= 3:
                    code, explanation = self._extract_code_and_explanation(parts)
                    break
...
<<<<<<< Tabnine <<<<<<<
    ...
    def _determine_source_type(self, source: str) -> str:#+
        ...
        if source.startswith(("http://", "https://")):#+
            return "doc"#+
        return "file"#+
>>>>>>> Tabnine >>>>>>># {"conversationId":"985550e3-a259-44e0-a379-9463bee5a042","source":"instruct"}
        """Determine the type of knowledge source"""
        if os.path.isfile(source):
            return "file"
        if os.path.isdir(source):
    ...
    def _acquire_from_file(self, file_path: str) -> List[Dict[str, Any]]:
        """Acquire knowledge from a file"""
            ...
            prompt = self._create_knowledge_extraction_prompt(content, file_path)
    ...
    def _acquire_from_doc(self) -> List[Dict[str, Any]]:
    ...
    def _detect_language(self, file_path: str) -> str:
        ...
        language_map = {
            ...
            ".go": "Go",
            ".rs": "Rust",
    def _create_knowledge_extraction_prompt(self, content: str, file_path: str) -> str:
        """Create prompt for knowledge extraction"""
