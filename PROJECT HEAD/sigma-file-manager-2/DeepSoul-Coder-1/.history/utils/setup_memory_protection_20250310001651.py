...
def load_memory_config(config_path: str = None) -> dict:
    """
    Load memory configuration from a JSON file
    ...
    if not config_path:
        ...
        locations = [
            "deepsoul_config/memory_config.json",
            os.path.expanduser("~/.deepseek-coder/memory_config.json"),
            "memory_config.json"
        ]
    ...
    if config_path and os.path.exists(config_path):
        try:
            ...
            if "memory_protection" in config:
                default_config["memory_protection"].update(config["memory_protection"])
            
            logger.info(f"Loaded memory configuration from {config_path}")
            return default_config
        except Exception as e:
            logger.error(f"Error loading memory configuration: {e}")
