...
class EvidenceProcessingService:
    ...
    def __init__(self, db_session: AsyncSession, fileboss_config: Optional[Dict[str, Any]] = None):
    ...
    async def process_evidence_directory(
        ...
        for file_meta in processed_files:
            try:
                ...
                logger.info(f"Processed evidence file: {file_meta.path} (ID: {evidence.id})")
                
            except Exception as e:
                logger.error(f"Error processing file {file_meta.path}: {e}", exc_info=True)
                continue
        ...
        for evidence_id in evidence_ids:
            ...
            if evidence and evidence.file_path:
                evidence_list.append({
                    ...
                    'modified': datetime.timestamp(evidence.updated_at or evidence.created_at)
                })
    ...
    async def link_evidence_to_timeline(
        ...
        try:
            ...
            if evidence:
                ...
                logger.info(f"Linked evidence {evidence_id} to timeline event {timeline_event_id}")
            
            return True
            
        except Exception as e:
            logger.error(
                f"Error linking evidence {evidence_id} to timeline event {timeline_event_id}: {e}",
                exc_info=True
            )
            return False
