...
from ...models import EvidenceCreate, EvidenceUpdate
from ..models import Evidence, EvidenceStatus, EvidenceType, timeline_event_evidence
...
class EvidenceRepository(BaseRepository[Evidence, EvidenceCreate, EvidenceUpdate]):
    ...
    async def get_by_type(
        self, evidence_type: EvidenceType, *, skip: int = 0, limit: int = 100
    ) -> List[Evidence]:
        """
        ...
        """
        return await self.get_multi(evidence_type=evidence_type, skip=skip, limit=limit)

    async def get_by_status(
        self, status: EvidenceStatus, *, skip: int = 0, limit: int = 100
    ) -> List[Evidence]:
        """
        ...
        """
        return await self.get_multi(status=status, skip=skip, limit=limit)


class EvidenceRepositoryAsync(
    EvidenceRepository, BaseRepositoryAsync[Evidence, EvidenceCreate, EvidenceUpdate]
):
    """
    ...
    async def add_to_timeline(self, evidence_id: str, timeline_event_id: str) -> bool:
        """
        ...
        stmt = insert(timeline_event_evidence).values(
            evidence_id=evidence_id, timeline_event_id=timeline_event_id
        )
    ...
    async def remove_from_timeline(self, evidence_id: str, timeline_event_id: str) -> bool:
        """
    ...
    async def add_custody_event(
        self, evidence_id: str, event: Dict[str, Any], user_id: str
    ) -> bool:
        """
        ...
        event.update({"timestamp": datetime.utcnow().isoformat(), "user_id": user_id})
...
class EvidenceRepositorySync(
    EvidenceRepository, BaseRepositorySync[Evidence, EvidenceCreate, EvidenceUpdate]
):
    """
